...
- **agent 自扫主路径**：不依赖 LLM 抽取管线；LLM 流水线产物（如有）只能当侦察线索，逐条 grep 核实后采用
...
scripts/
  ...
  export_dumps.py             # 旧流水线索条导出（模式二备用）
```
...
- 《弱角友崎同学》（110 万字）：379 条行号引用抽查 8/8 命中；揪出流水线幻觉实体 4 个、SPLIT-PAIR 碎片 2 组
- 《有谁规定了现实中不能有恋爱喜剧的？》（6 卷完结）：六类雷点零检出的干净书，裁定"怪盗=曲名非别名"等 4 处易错点
- 《北美悍警》（1086 章未完结，22.8 万行）：24 单元 10 角色，引文抽查 14/14 命中

## 声明

- 产出的扫书报告对外传播时必须保留「AI 辅助生成」标注
- 雷点/郁闷点判定口径致敬 [novel-digest](https://github.com/) 项目的 rules.json 分类学
