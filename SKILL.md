...
description: |
  ...
  雷点/郁闷点判定遵循内置分类学（口径源自 novel-digest 项目）（见 references/leidian-taxonomy.md），禁止泛化为"读起来难受的情节"。
...
- **novel-digest（可选）**：见"路径选择"；主路径完全不依赖它。

## 路径选择

- **主路径（默认）：agent 自扫**。子代理直接 Read/Grep 原文。
- **可选加速：novel-digest 产物只当侦察线索**（漏女补漏、雷点候选），每条必须 grep 原文核实。若使用者本机装有 novel-digest 则读其 results/，没装则完全跳过，主路径不依赖它。
